...
step1:
    ...
    res = nfc_pcd_decode_data(&g_nfc_pcd_signal_data);
    memcpy(tag_clg, (const void *)g_nfc_pcd_signal_data.decode_buf.v8, 4);

    nfc_pcd_cypto_create(pKey, pUid, tag_clg, reader_clg, reader_rsp, tag_rsp, oddparity, g_nfc_pcd_signal_data.is_encrypted);

    memcpy((void *)g_nfc_pcd_signal_data.decode_buf.v8, (const void *)reader_clg, 4);
    memcpy((void *)&g_nfc_pcd_signal_data.decode_buf.v8[4], (const void *)reader_rsp, 4);
...
step2:
    ...
    res = nfc_pcd_decode_data(&g_nfc_pcd_signal_data);
    if (memcmp((const void *)tag_rsp, (const void *)g_nfc_pcd_signal_data.decode_buf.v8, 4) == 0)
    {
        res = PCD_NO_ERROR;
    }
    else
...
step2:
    ...
    memcpy((void *)g_nfc_pcd_signal_data.decode_buf.v8, (const void *)pData, 16);
...
step2:
    ...
    memcpy((void *)g_nfc_pcd_signal_data.decode_buf.v8, (const void *)pValue, 4);
